...
parser.add_argument("-v", "--video", type=str, help= "path to video", default=os.path.join("sample_data2/complex_video.dav"))
...
ct.update(result.boxes.data.tolist())
trackers = []
TrackableObjects = {}
...
while True:
    ...
    new_edges, new_edges_as_box = get_edges(result, edges2, edges_as_box, detection_threshold=detection_threshold)
    # edges2, frame_number, edges_as_box = get_edges_from_cap(cap=cap2, model=model, \
    #    frame_number=frame_number, edges2=edges2, detection_threshold=detection_threshold)

    # edges_list = list(edges[0][0])
    mask = np.zeros_like(frame_gray_init)
    
    for edge in new_edges_as_box:
        print('heyho2')
        # sleep(1)
        print(edge)
        x1, y1, x2, y2 = edge
    ...
    # new_edges_list = list(new_edges[0][0])
    # edges_list.extend(new_edges_list)
    # edges[0][0] = np.array(new_edges_list, dtype=np.float32)
    ...
    new_edges_test = cv2.goodFeaturesToTrack(frame_gray, mask=None, **parameters_shitomasi)    
    new_edges = cv2.goodFeaturesToTrack(frame_gray, mask=mask, **parameters_shitomasi)
    ...
    cv2.imshow('Optical Flow (sparse)', result_image)
    # cv2.imshow('Window name', frame)
