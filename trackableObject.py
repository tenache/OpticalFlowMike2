...
class CentroidTracker():
    def __init__(self, maxDisappeared=50):
        ...
        # used to keep tracke of mapping a given object ID to
        # its centroid and number of consecutive frame it has been marked as 
        # "disappeared", respectively
        self.nextObjectID = 0
        self.objects = OrderedDict()
        self.disappeared = OrderedDict()
        
        # store the number of maximum consecutive frames a given object
        # is allowed to be marked as "disappeared until we need to 
        # # deregister the object from tracking
        # self.maxdisappeared = maxDisappeared
    
    def register(self, centroid):
        # when registering an object we ues the next available object
    ...
    def update(self, rects):
        ...
        # otherwise, are currently tracking objects wo we ned to try to match the input
        # centroids to existing object centroids
            ...
            # the smallest value in each column and then sorting using the previously
            # computed row index list
            cols = D.argmin(axis=1)[rows]
            
            # in order to determine if we need to update, register, or deregister 
            # an object we need to keep track of which of the rows and column indexes
            ...
            for (row, col) in zip(rows, cols):
                # if we have already examined eiher the row or column
                # value before, ignore it val
                if row in usedRows or col in usedCols:
                    continue
                
                # otherwise, grab the object ID for the current row, set
                # its new centroid, and reset the disappeared counter
                ...
                # in the event that the number of object centroids is equal
                # or greater than the number of input centroids we need to check
                ...
                if D.shape[0] >= D.shape[1]:
                    ...
                    for row in unusedRows:
                        ...
                        # check to see if the number of consecutive frames the object
...
# WHAT IS WRONG ?? WHAT DO I HAVE TO CHANGE??? 
...
class TrackableObject:
    def __init__(self, objectID, centroid):
        ...
        # initialize a booean used to idndicate if th e ojbect has
